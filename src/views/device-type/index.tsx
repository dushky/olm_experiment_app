...
import { DataGrid, GridColumns,GridCellEditCommitParams, GridToolbarContainer, GridToolbarExport, gridClasses } from "@mui/x-data-grid"
import AddDeviceTypeForm from './AddDeviceTypeForm'
import { gridSpacing } from 'assets/constants'
import DeleteIcon from '@mui/icons-material/Delete'
import { DeviceTypeDataFragment, useUpdateDeviceTypeMutation, useRemoveDeviceTypeMutation, useCreateDeviceTypeMutation} from "generated/graphql"
...
const DeviceType = ({deviceType}: Props) => {
                ...
                <Button
                    ...
                    onClick={async () => {
                        await removeDeviceType({
                            ...
                            setRows(rows.filter((item) => item.id != removedDeviceType.data?.removeDeviceType.id))
                            setOpen(true)
                        })
                    }}
                >
    ...
    const exportToolbar = () => {
        return ( <GridToolbarContainer className={gridClasses.toolbarContainer}>
            <GridToolbarExport />
    ...
    const handleSubmit = async (value: string) => {
        await createDeviceType({
            ...
            setRows([...rows, {id: value.data!.createDeviceType.id, name: value.data!.createDeviceType.name}])
            setOpen(true)
        })
    }
